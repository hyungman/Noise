...
void syntax(const char *a) {
  ...
  cerr
      << "  -t dt nt           time series delta, number of timesteps (0, 1)\n";
...
int main(int argc, char *argv[]) {
  ...
  for (int i = 1; i < args.size(); i++) {
    if (args[i] == "-h") {
      ...
      psize = std::stoi(args[++i]);

    } else if (args[i] == "-m") {
      mpir = std::stoi(args[++i]);
      mpis = std::stoi(args[++i]);
      ...
      pers = std::stof(args[++i]);

    } else if (args[i] == "-F") {
      freq = std::stof(args[++i]);

    } else if (args[i] == "-O") {
      octave = std::stoi(args[++i]);

    } else if (args[i] == "-t") {
      delta_t = std::stof(args[++i]);
      nt = std::stoi(args[++i]);
  ...
  for (int t = 0; t < nt; t++) {
    if (mpir == 0) {
      ...
      std::string prefix_pvti_filename(pvti_filename);
      prefix_pvti_filename = prefix + prefix_pvti_filename;
      pvti.open(prefix_pvti_filename);

      pvti << "<?xml version=\"1.0\"?>\n";
      pvti << "<VTKFile type=\"PImageData\" version=\"0.1\" "
           << "byte_order=\"LittleEndian\" "
              "compressor=\"vtkZLibDataCompressor\">\n";
      pvti << "  <PImageData WholeExtent=\" 0 " << (zsz - 1) << " 0 "
           << (ysz - 1) << " 0 " << (xsz - 1)
           << "\" GhostLevel=\"1\" Origin=\"0 0 0\" "
           << " Spacing=\"" << d << " " << d << " " << d << "\">\n";
      pvti << "    <PPointData Scalars=\"noise\" Vectors=\"gradient\">\n";
      pvti << "      <PDataArray type=\"Float32\" Name=\"noise\"/>\n";
      pvti << "      <PDataArray type=\"Float32\" Name=\"gradient\" "
    ...
    for (int part = 0; part < nparts; part++) {
      ...
      int sx = ix * dx, sy = iy * dy, sz = iz * dz;
      ...
      std::string  prefix_vti_partname(vti_partname);
      prefix_vti_partname = prefix + vti_partname;

      if (mpir == 0)
        pvti << "<Piece Extent=\"" << sz << " " << ez << " " << sy << " " << ey
             << " " << sx << " " << ex << "\" Source=\"" << prefix_vti_partname
             << "\"/>\n";

      if ((part % mpis) == mpir) {
        int lxsz = (ex - sx) + 1, lysz = (ey - sy) + 1, lzsz = (ez - sz) + 1;
        ...
        int xstep = lzsz * lysz, ystep = lzsz, zstep = 1;
        ...
        wr->SetInputData(id);
        wr->SetFileName(prefix_vti_partname.c_str());
